...
#region LaunchMQTT
	...
	static void EdgeXAIclient_MqttMsgPublishReceived(object sender, MqttMsgPublishEventArgs e) {
		try {
			...
			if(e.Topic == MqttTopic[0]) {
				...
				redMessage($"Received other topic \"{e.Topic}\"");
			}
		}
		catch (Exception ex) {
			redMessage("=============Exception Occurs Start============= ");
			redMessage($"ex.ToString()");
			redMessage("==============Exception Occurs End============== ");
			return;
		}
	}
	...
	static void LaunchMQTT() {
		try {
			...
			EdgeXAIclient.Subscribe(MqttTopicAll, MqttQos);
		}
		catch (Exception ex) {
			redMessage("=============Exception Occurs Start============= ");
			redMessage($"ex.ToString()");
